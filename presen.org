...
+ Action: $\text{Eat}(\text{pizza🍕})$ 
+ Precondition: $\lnot \text{full}() \land \text{have}(\text{pizza🍕})$
+ Effects: *Delete effects:* $\lnot\text{have}(\text{pizza🍕})$ : -- in the stomach now
...
+
  ...
  #+begin_larger
  #+begin_center
  Not PDDL compatible → */Can't use a fast solver/*
  #+end_center
  #+end_larger
...
* To use a fast planner we need PDDL @@html:<br>@@ → We should */embed PDDL grammar in NN model/*
...
+
  ...
  　 I */like/* an apple : verb

  　 Thanks for the */like/* on Facebook! : noun
...
#+begin_larger
#+begin_alignright
+ → symbols in *different namespace* requires
...
*Problem symbol:* → Different search space

　Stage 1-1, Stage 1-2, ... in Super Mario Bros.
...
*Domain symbol:*  → Different set of available actions
...
#+begin_alignright
+ → Planning + Learning that is not *Representation Learning*
#+end_alignright
